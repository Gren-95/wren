...
impl WrenWindow {
    ...
    /// banner.
    pub fn show_banner(&self, text: &str, button_label: Option<&str>, action_name: Option<&str>) {
    ...
    fn run_search(&self, query: &str) {
        ...
        self.show_banner("Searching subfolders…", Some("Cancel"), Some("cancel-search"));
        content_stack.set_visible_child_name("files");
        status_bar.set_text("");
        glib::spawn_future_local(glib::clone!(
            #[weak(rename_to = window)]
            ...
            async move {
                ...
                if !is_current { return; }
                match result {
                    Ok(true) => window.show_banner(
                        "Search truncated — too many results",
                        None,
                        None,
                    ),
                    Ok(false) => window.hide_banner(),
                    Err(_) => window.hide_banner(),
                }
